...
export function sidesPropertyRule (propertyName: string): DynamicMatcher {
    return ([_, side, value]: string[]): CSSEntries | undefined => {
        ...
        const getComputedSize = (propertyName: string, side: string, size: string):string => {
            const variable = sidesPropertyKeys.includes(side) ? `var(--${propertyName}-${side})` : 'var(--spacing)';

            return /^-?\d+(\.\d+)?(\/\d+)?$/.test(size)
                ? (size === '1' ? variable : `calc(${variable} * ${size})`)
                : size;
        };

        if (value) {
            if (side) {
                return { ...sidesMap, ...modifierMap }[side].map((sideKey) => [`${propertyName}-${sideKey}`, getComputedSize(propertyName, sideKey, value)]);
            }
